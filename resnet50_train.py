# In this kernel, we will use the Resnet50 model provided in the Pytorch standard library pretrained on imagenet for a baseline prediction model
...
#semantic_map_key = core_dataset/semantic_map/semantic_map.pb
...
flags_dict = {
    "debug": True,
    # --- Data configs ---
    "l5kit_data_folder": dataset_path,
    # --- Model configs ---
    "pred_mode": "multi",
    # --- Training configs ---
    "device": device,
...
l5kit_data_folder = flags_dict["l5kit_data_folder"]
...
len_train_dataset = int(len(train_agent_dataset) * train_cfg['dataset_coverage'])
train_dataset = Subset(train_dataset, np.arange(len_train_dataset))
...
if debug:
    ...
    # Only use 5000 dataset for fast check...
    valid_dataset = Subset(valid_dataset, np.arange(3000))
...
# download log file
log_path = "../input/resnet50-training/results/multi_train/log.csv"
df = pd.read_csv(log_path)
plt.plot(df['iteration'], df['main/loss'])
print(df)
